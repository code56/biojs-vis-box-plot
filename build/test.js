...
module.exports = biojsvisboxplot = function (init_options)
{
    ...
    sort_x_by_probe_and_disease_state = function (graph) {
        ...
        if (options.disease_state_order !== 'none') {
            disease_state_order = options.disease_state_order.split(',');
            ...
            nested_values = d3.nest()
                    ...
                    .sortKeys(function (a, b) {
                        return disease_state_order.indexOf(a) - disease_state_order.indexOf(b);
                    })
                    .key(function (d) {
                        return d.Sample_Type;
                    })
            ...
            if (sort_by_diseases === true) { //There will be a second key which is the disease stat
                for (disease_states in values) {
                    ...
                    for (sample_types in disease_values) {
                        ...
                        graph = draw_box_plot(expression_values, graph, box_plot_vals, parseInt(probe), parseInt(disease_states), parseInt(sample_types), number_sample_types, probe_name, sample_type, disease_state);
                    }
                }
            } else {
                ...
                for (sample_types in values) {
                    ...
                    // Actually draw the box plot on the graph
                    graph = draw_box_plot(expression_values, graph, box_plot_vals, parseInt(probe), 0, parseInt(sample_types), number_sample_types, probe_name, sample_type, disease_state_names);
                }
            }
        }
        graph.sample_type_list = sample_type_list;
    ...
    make_box_tooltip = function (probe, sample_type, disease_state) {
	...
	var multiplier = 1.5;// (used for making sure the line is centered)
        ...
        scaleX = graph.scaleX;
        options = graph.options;
        tooltip_box = make_box_tooltip(probe_name, sample_type_name, disease_state_name);
