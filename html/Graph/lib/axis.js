    ...
    */
    setup_data_for_x_axis = function(graph){
        ...
        var sort_by_second_sort_bys = false;
        if (options.multi_group != 1) {
            var sort_by_multiple = true;
        }
        var sample_id_list = [];
        var sample_type_list = [];
        var second_sort_by_list = [];
        // This allows us to have a scale for the x axis no matter how many
        // different samples
        var multi_xaxis_scale = [];
        var vertical_lines_mapping = [];
        var second_sort_by_count = 0; //Keeps track of how many second_sort_by types there are so we can evenly
  ...
  /**
    ...
    return_y_min_max_values = function (graph) {
        ...
        var lwr_min_max_values_from_data = d3.extent(options.data,
                function (d) {   // this will go through each row of the options.data
                    ...
                    var lwr = (d.Expression_Value - d.Standard_Deviation);
                    var temp = lwr; // this will get the y_column (usually prediction) from the row
        ...
        var upr_min_max_values_from_data = d3.extent(options.data,
                function (d) {
                    var extra_padding_for_y_axis = 1;
                    // changes reverted for extra padding
                    var upr = (d.Standard_Deviation + d.Expression_Value) ;
        ...
        var min = lwr_min_max_values_from_data[0];

        var max = upr_min_max_values_from_data[1];
        ...
        for (var key in options.horizontal_lines) {
            var value = options.horizontal_lines[key];
    ...
    setup_y_axis = function (graph) {
        ...
        var y_axis_legend_y = (graph.full_height - options.margin.top - options.margin.bottom) / 2;
    ...
    setup_x_axis = function (graph, sample_list) {
        ...
        var font_size = "0px"; // set this to 0 if you don't want sample_id as the labels on the x axis
        svg.append("g")
                .attr("class", "x_axis")
    ...
    calc_x_value_label = function (graph, d, i, info) {
            ...
            if (cur_line != prev_line) {
                var avg = graph.multi_scaleX(graph.multi_xaxis_list[i]);
                prev_line = cur_line;
          ...
          if( option == 1) // returns gene and probe name STAT1 ILMN_1543256
            {
                  if(graph.graph_type == "Scatter Plot" || label_num == 3 || label_num == 2) {
                    ...
                    if(ref_type == "gene_set_id" || ref_type == "probeID") {
                      ...
                      prefix = options.ref_name[options.box.symbol];
                    }
                  }
                  var word = prefix + " " + temp;
