...
module.exports = biojsvisscatterplot = function(init_options)
{
 ...
 /*------------------------------Box plot Calculations--------------------------------------*/
    ...
    this.draw_box_plot = function(graph, box_plot_vals, probe, disease_state, sample_type, number_sample_types, probe_name, disease_state_name, sample_type_name) {
        svg = graph.svg;
        scaleY = graph.scaleY;
        scaleX = graph.scaleX;
        options = graph.options;
        ...
        x_buffer = (probe_size * probe) + (disease_state_size * disease_state) + (sample_type_size * (sample_type)) + (sample_type_size * 0.5);
        ...
        svg.append("rect")
            ...
            .on("mouseout", tooltip.hide);
                //Add min line
        svg = this.add_line_to_box(options.stroke_width, x_buffer, box_width, box_plot_vals[0], svg, scaleY, colour_wiskers);
        //Add median line
        svg = this.add_line_to_box(options.stroke_width, x_buffer + box_width*.25, box_width*0.5, box_plot_vals[2], svg, scaleY, colour_median);
        //Add max line
        svg = this.add_line_to_box(options.stroke_width, x_buffer, box_width, box_plot_vals[4], svg, scaleY, colour_wiskers);
        graph.svg = svg;
        return graph;
    }
    
    this.add_line_to_box = function(stroke_width, x_buffer, box_width, y_value, svg, scaleY, colour) {
        svg.append("line")
                .attr("x1", (x_buffer - box_width/2))
                .attr("x2", (x_buffer + box_width* 1.5))
