...
module.exports = biojsvisscatterplot = function(init_options)
{
   ...
   // Sorts the sorted probe types by disease state if necesary so that they can
    ...
    this.sort_x_by_probes = function(graph) {
        options = graph.options;
    ...
    this.calculate_x_value_of_probes = function(graph) {
        ...
        section_size = (width/probe_count)/2;
    ...
    this.setup_disease_state_labels = function (graph) {
        ...
        for (probe in vertical_lines) {
            padding = full_size_of_a_probe_collumn * count;
            ...
            for (disease in disease_state_list) {
                ...
                svg.append("text") // when rotating the text and the size
                    ...
                    .attr("x",
                        function(d){
                           x_value = (size_of_disease_state_collumn * (count + 0.5)) + padding;
                           return x_value;
                        }
 ...
 /*------------------------------Box plot Calculations--------------------------------------*/
            ...
            if (sort_by_diseases == true) { //There will be a second key which is the disease state
                for (disease_states in values) {
                    ...
                    for (sample_types in disease_values) {
                        ...
                        sample_values = row.values;
                        sample_type = row.key;
                        //At the level of the xcel file now
                        expression_values = [];
                        for(i in sample_values) {
                            data_row = sample_values[i];
                            data = data_row.values;
                            sample_type = data_row.key;
                            for (x in data) {
                                expression_values.push(data[x].Expression_Value);
                            }
                        ...
                        graph = this.draw_box_plot(graph, box_plot_vals, probe, sample_types, disease_states, (probe_name + "-" + sample_type + "-" + disease_state));
                ...
                for(sample_types in values) {
                    sample_row = sample_types[sample];
                    sample_values = row.values;
                    sample_type = row.key;
                    //At the level of the xcel file now
                    expression_values = [];
                    for(i in sample_values) {
                        expression_values.push(i.Expression_Value);
                    }
                    box_plot_vals = this.calculate_box_plot_vals(expression_values);
                    // Actually draw the box plot on the graph
                    graph = this.draw_box_plot(graph, box_plot_vals, probe, sample_types, 0, (probe_name + "-" + sample_type + "-" + disease_state));
                }
            }
        }
        return graph;
    ...
    this.draw_box_plot = function(graph, box_plot_vals, probe, sample_type, disease_state, id) {
        svg = graph.svg;
        scaleY = graph.scaleY;
        scaleX = graph.scaleX;
        options = graph.options;
        ...
        probe_size = graph.size_of_probe_collumn * 2;
        disease_state_size = graph.size_of_disease_state_collumn * 2;
        x_buffer = (probe_size * (probe)) + ((disease_state) * (disease_state_size));
        //Add min line
        svg = this.add_line_to_svg(options.stroke_width, x_buffer, box_width, box_plot_vals[0], svg, scaleX, scaleY);
        //Add median line
        svg = this.add_line_to_svg(options.stroke_width, x_buffer, box_width, box_plot_vals[2], svg, scaleX, scaleY);
        //Add max line
        svg = this.add_line_to_svg(options.stroke_width, x_buffer, box_width, box_plot_vals[4], svg, scaleX, scaleY);
        //Add box
        svg.append("rect")
            .attr('width', box_width)
            .attr('x', x_buffer)
            .attr('id', id)
            .attr('y', scaleY(box_plot_vals[1]))
            .attr('height', scaleY(box_plot_vals[3] - box_plot_vals[1]))
    ...
    this.add_line_to_svg = function(stroke_width, x_buffer, box_width, y_value, svg, scaleX, scaleY) {
        svg.append("line")
                .attr("x1", (x_buffer - box_width))
    ...
    this.setup_vertical_lines = function(graph){
        ...
        sample_id_list = graph.sample_id_list;
        page_options = graph.page_options;
        size_of_probe_collumn = graph.size_of_probe_collumn * 2; //Returns half size of a probe collumn
    ...
    this.setup_box_plot_wiskers = function (graph){
        svg = graph.svg;
        options = graph.options;
        page_options = graph.page_options;
        scaleX = graph.scaleX;
	...
	legend.append('text')
		...
		.attr('y', legendRectSize - legendSpacing)
	        .style("font-family", options.font_style)
            	.style("font-size", options.text_size)
		.style('opacity', 1)
		.text(function(probeInfo) { return probeInfo[0]; });
