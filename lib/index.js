...
module.exports = biojsvisscatterplot = function(init_options)
{
    ...
    this.setup_scatter_line = function(graph){
        ...
        dataNest.forEach(function(d,i) {
         svg.append("path")
            .attr("class", function() {
                        return "line-probe line-probe-" +  d.key.replace(/\ |(|)/g,''); })
    ...
    */
    this.setup_data_for_x_axis = function(graph){
        ...
        for (probe in nested_values) {
            ...
            key = row.key;
            values = row.values;
            // Resets the first disease state so we know to start with the 0th one
            fisrt_disease_state = true;
            for (i in values) {
                sample = values[i];
                probe = sample.Probe;
                ...
                if (sort_by_diseases == true) {
                    if ($.inArray(disease_state, disease_state_list) == -1) {
                        if (first_disease_state == true) {
                            start_sample_id = values[0].Sample_ID;
                        }
                        temp_probe['disease_state'] = disease_state;
                        temp_probe['start_disease_state'] = start_sample_id;
                        temp_probe['end_disease_state'] = sample_id;
                        disease_state_list.push(temp);
                        disease_state_count ++;
            ...
            temp['probe'] = key;
            temp['start_probe'] = values[0].Sample_ID;
            temp['end_probe'] = sample_id;
            temp['disease_state_list'] = disease_state_list;
            vertical_lines.push(temp);
            line_count ++;
        }
        graph.probe_count = probe_count;
   ...
   // Sorts the sorted probe types by disease state if necesary so that they can
    ...
    this.sort_x_by_probe_and_disease_state = function(graph) {
        ...
        graph.nested_values = nested_diseases;
        return graph;
    }
    ...
    this.calculate_x_value_of_disease_state = function(graph) {
        ...
        section_size = ((width/probe_count)/disease_state_count)/2;
        graph.size_of_disease_state_collumn;
        return graph;
    } // calculate_x_value_of_probes
    ...
    this.setup_disease_state_labels = function (graph) {
        ...
        full_size_of_a_probe_collumn = 2 * graph.size_of_probe_collumn;
        count = 0;
        for (probe in vertical_lines) {
            padding = full_size_of_a_probe_collumn * count;
            svg.selectAll(".disease_state_text")
                .data(probe.disease_state_list).enter()
                ...
                .attr("x",
                    function(d, i){
                       x_value = (size_of_disease_state_collumn * (i + 1)) + padding;
                       return x_value;
                    }
 ...
 /*------------------------------Box plot Calculations--------------------------------------*/
    ...
    this.return_min_max_vals = function(values_for_sample_type) {
        options = graph.options;
        max_val = 1;
        min_val = 0;

        for (sample_value in values_for_sample_type) {
            if (sample_value < min_val) {
                min_val = sample_value;
            }
            if (sample_value > max_val) {
